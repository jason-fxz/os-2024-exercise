...
void test_mmap_remap() {
    ...
    assert(addr1 != addr2), "Virual addresses are same";
    assert(get_physical_address(addr1) != get_physical_address(addr2)), "Physical addresses are same";
...
void test_file_operations(const char* filename, size_t filesize) {
    ...
    assert(file_mmap_write(filename, filesize - strlen(content3), content3) == 0);
    ...
    if (fp) {
        ...
        fseek(fp, filesize - strlen(content3), SEEK_SET);
